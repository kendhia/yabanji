...
def open_csv_file(file_name, seperator, nameslist):
        '''df = pd.read_csv('/home/fykendhia/Documents/yabanji/social_mapper/scripts/data.csv',
                        sep=',',
                        header=None,
...
def stem_data(df, value_label):
        ...
        df[value_label] = df.tweet.str.replace('[^\w\s]', '')
        df[value_label] = df[value_label].apply(word_tokenize)
        df[value_label] = df[value_label].apply(lambda x : [stemmer.stem(y) for y in x])
...
def create_model(counts, df, class_label):
        X_train, _, y_train, _ = train_test_split(counts, df[class_label], test_size=0.1, random_state=69)
...
def predect_from_df(csv_name, value_label):
        ...
        with open("/home/fykendhia/Documents/yabanji/social_mapper/scripts/model", "rb") as f:
                model = cPickle.load(f)
                bow_transformer = cPickle.load(f)

        stemed_df = stem_data(df, value_label)
        _, counts_vector = convert_to_tfidf(stemed_df, value_label, bow_transformer)
        predicted = model.predict(counts_vector)
        return predicted


def new_model():
        df = open_csv_file('/home/fykendhia/Documents/yabanji/social_mapper/scripts/data.csv',',', ['label','id','datetime','source','username','tweet'])
